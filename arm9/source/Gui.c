...
#define EMUVERSION "V0.3.2 2024-03-21"
...
const fptr fnList9[] = {quickSelectGame, quickSelectGame, quickSelectGame};
...
const char *const bonusTxt[] = {"30K 70K+", "40K 80K+"};
...
void uiAbout() {
	...
	drawMenuText("DPad:   Move ship", 6, 0);
	drawMenuText("B:      Shoot", 7, 0);
	drawMenuText("A:      Special attack", 8, 0);

	drawMenuText("FinalizerDS  " EMUVERSION, 20, 0);
...
void uiLoadGame() {
	...
	int i;
	for (i=0; i<ARRSIZE(finalizrGames); i++) {
		drawSubItem(finalizrGames[i].fullName, NULL);
	}
}
